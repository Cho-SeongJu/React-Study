...
export default function RadioGroup({ children, name, onChange }) {
  ...
  const updateGroupStates = (updatedRadio) => {
    const newStates = states.map((s) => (s.id === updatedRadio.id ? updatedRadio : { ...s, selected: false }));
    setStates(newStates);
  };
  ...
  return <RadioGroupContext.Provider value={{ selectedId, updateGroupStates }}>{children}</RadioGroupContext.Provider>;
}
